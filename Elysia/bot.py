...
class Elysia(Client):
    def __init__(self):
        ...
        super().__init__(
            name=name,
            app_version=f"Elysia r{__version_number__} ({__commit__})",
            api_id=API_ID,
            api_hash=API_HASH,
            bot_token=TOKEN,
            parse_mode=ParseMode.HTML,
            workers=WORKERS,
            plugins={"root": "Elysia.plugins", "exclude": DISABLED_PLUGINS},
            sleep_threshold=180,
        )
    ...
    async def start(self):
        ...
        logger.info(
            "Elysia running with Pyrogram v%s (Layer %s) started on @%s. Hi!",
    ...
    async def stop(self):
        await super().stop()
        logger.warning("Elysia stopped. Bye!")
